...
#   ./run_pipeline.sh --new-tunnel (Re)start with a fresh tunnel URL (re-syncs Lovable once).
...
# same URL, so Lovable is NOT rebuilt every restart. Only --new-tunnel / a reboot changes it.
set -euo pipefail
cd "$(dirname "$0")"
...
for arg in "$@"; do
  case "$arg" in
    --restart|--serve-only) PIPELINE=skip ;;
...
# Use plain docker if the daemon is reachable, else fall back to sudo.
DOCKER="docker"
...
# Postgres is needed for both the pipeline and the API — always ensure it's up.
